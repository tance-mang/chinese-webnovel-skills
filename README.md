...
[![Skills](https://img.shields.io/badge/skills-32-brightgreen)](#-32-个技能)
...
```
首次会问你要 API Key（DeepSeek/Kimi/硅基流动等，有免费额度）；没有先回车跳过，弄到再来。进去后：
```text
/help          看所有命令
...
```
装好重启，输入 `/webnovel:idea` 看到技能即成功。直接说"帮我写个修真开头"它会自己调技能。
...
├── skills/                  # 32 个技能（draft 含 build_docx.py 生成 Word）
├── references/              # 31 个知识库
